            ...
            <nav className="py-4 px-1 flex flex-col-reverse items-center md:flex-row justify-around">
                <div className="flex gap-5 items-center h-5 justify-center">    
                    {categories.slice(0,5).map(({slug, name})=>{
                        return(
                           <Link className="text-md hover:border-b-4 border-cyan-600 sm:text-md md:text-md xl:text-xl" href={`/category/${slug}`} key={slug}>{name}</Link>
                        )
                    })}
                </div>
                <TempState/>
                        ...
                        return(
                         <div key={item.slug} className="flex flex-col items-center justify-center w-full">
                            <div className="flex w-full gap-5" id={item.name}>
                              <Link onClick={handleShowPortal} className={`w-full text-md border-b-2 font-bold hover:text-blue-500 sm:text-md md:text-lg`} href={`/category/${item.slug}`}>{item.name}</Link>                              
